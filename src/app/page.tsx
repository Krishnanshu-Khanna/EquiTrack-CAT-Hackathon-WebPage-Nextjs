      ...
      <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {anomalies.filter(a => a.severity === 'High').length} high severity alerts
            </p>
          </CardContent>
        </Card>
      </div>
             ...
             {anomalies.slice(0,4).map(anomaly => (
              ...
              <div className="grid gap-1">
                <p className="text-sm font-medium leading-none">
                  {anomaly.type} on {equipment.find(e => e.id === anomaly.equipmentId)?.name}
                </p>
                <p className="text-sm text-muted-foreground">
                  {new Date(anomaly.timestamp).toLocaleString()}
                </p>
              </div>
              <Badge variant="destructive" className="ml-auto sm:hidden">{anomaly.severity}</Badge>
            </div>
             ))}
          </CardContent>
        </Card>
